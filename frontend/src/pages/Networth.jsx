...
// ─── Timeline builder ────────────────────────────────────────────────────────────────────────────
// The backend pushes every buy/sell/update as a new value_history entry with
// the event date and cumulative current_value. Fallback: use the FIRST history
// entry's value (= original invested_value from createAsset), NOT
// asset.invested_value which grows with every buy and inflates earlier points.
...
function buildTimeline(assets, liabilities) {
  ...
  return dates.map((date) => {
    ...
    return { date, label: fmtDate(date), totalAssets: Math.round(totalAssets), totalLiabilities: Math.round(totalLiabilities), netWorth: Math.round(totalAssets - totalLiabilities) };
  });
}
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div style={tooltipStyles.wrap}>
      ...
      <div style={tooltipStyles.row}><span style={{ ...tooltipStyles.dot, background: "#6366f1" }} /><span style={tooltipStyles.label}>Assets</span><span style={tooltipStyles.value}>{fmtFull(data.totalAssets)}</span></div>
      <div style={tooltipStyles.row}><span style={{ ...tooltipStyles.dot, background: "#f87171" }} /><span style={tooltipStyles.label}>Liabilities</span><span style={tooltipStyles.value}>{fmtFull(data.totalLiabilities)}</span></div>
      <div style={tooltipStyles.divider} />
      <div style={tooltipStyles.row}><span style={{ ...tooltipStyles.dot, background: gainColor(nw) }} /><span style={{ ...tooltipStyles.label, color: "#e2e8f0" }}>Net Worth</span><span style={{ ...tooltipStyles.value, color: gainColor(nw), fontWeight: 600 }}>{gainPrefix(nw)}{fmtFull(nw)}</span></div>
    </div>
  );
}
...
function BreakdownBar({ assets, liabilities }) {
  ...
  return (
    <div style={bbStyles.wrap}>
      <div style={bbStyles.header}><span style={bbStyles.title}>Portfolio Composition</span></div>
      ...
      <div style={bbStyles.legend}>
        <div style={bbStyles.legendItem}><span style={{ ...bbStyles.legendDot, background: "#6366f1" }} /><span style={bbStyles.legendLabel}>Assets</span><span style={bbStyles.legendVal}>{fmt(totalAssets)}</span><span style={bbStyles.legendPct}>{assetPct.toFixed(1)}%</span></div>
        <div style={bbStyles.legendItem}><span style={{ ...bbStyles.legendDot, background: "#f87171" }} /><span style={bbStyles.legendLabel}>Liabilities</span><span style={bbStyles.legendVal}>{fmt(totalLiab)}</span><span style={bbStyles.legendPct}>{liabPct.toFixed(1)}%</span></div>
      </div>
    </div>
  );
}
...
function SummaryList({ items, type }) {
  ...
  const sorted = [...items].sort((a, b) => isAsset ? (b.current_value || b.invested_value) - (a.current_value || a.invested_value) : b.current_balance - a.current_balance).slice(0, 5);
  const total = items.reduce((s, i) => s + (isAsset ? (i.current_value || i.invested_value || 0) : (i.current_balance || 0)), 0);
  return (
    <div style={slStyles.wrap}>
      <div style={slStyles.header}>
        <span style={{ ...slStyles.title, color: isAsset ? "#818cf8" : "#f87171" }}>{isAsset ? "▲ TOP ASSETS" : "▼ TOP LIABILITIES"}</span>
        ...
        return (
          <div key={item._id || i} style={slStyles.row}>
            <div style={slStyles.rowLeft}>
              ...
              <div><div style={slStyles.name}>{isAsset ? item.asset_name : item.liability_name}</div><div style={slStyles.sub}>{isAsset ? item.institution : item.lender}</div></div>
            </div>
            <div style={slStyles.rowRight}>
              <span style={{ ...slStyles.val, color: isAsset ? "#a5b4fc" : "#fca5a5" }}>{fmt(val)}</span>
              <div style={slStyles.miniBar}><div style={{ ...slStyles.miniFill, width: `${pct}%`, background: color }} /></div>
            </div>
          </div>
        );
      })}
...
const slStyles = {
  ...
  avatar: { width: 30, height: 30, borderRadius: 8, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontFamily: "'DM Serif Display', serif" },
      ...
      <div style={styles.container}>
        <div style={styles.pageHeader}>
          ...
          <div style={styles.netWorthDisplay}>
            ...
            <span style={{ ...styles.netWorthValue, color: gainColor(netWorth) }}>{fmtFull(netWorth)}</span>
            {change !== 0 && (
        ...
        <div style={styles.chartCard}>
          <div style={styles.chartHeader}>
            <div>
              ...
              <span style={styles.chartSub}>
                {chartData.length > 1 ? `${fmtDateLong(chartData[0].date)} — ${fmtDateLong(chartData[chartData.length - 1].date)}` : "Tracking begins when you add history"}
              </span>
            </div>
            <div style={styles.rangeRow}>
              {RANGES.map((r) => (
                <button key={r} style={{ ...styles.rangeBtn, ...(range === r ? styles.rangeBtnActive : {}) }} onClick={() => setRange(r)}>{r}</button>
              ))}
            </div>
          </div>
          ...
          {chartData.length < 2 ? (
            <div style={styles.chartEmpty}>
              ...
              <p style={styles.chartEmptyText}>Not enough history yet. Update asset values or make liability payments to build your timeline.</p>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={340}>
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                <defs>
                  <linearGradient id="gradAssets" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6366f1" stopOpacity={0.25} /><stop offset="95%" stopColor="#6366f1" stopOpacity={0.02} /></linearGradient>
                  <linearGradient id="gradLiab"   x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f87171" stopOpacity={0.2}  /><stop offset="95%" stopColor="#f87171" stopOpacity={0.02} /></linearGradient>
                  <linearGradient id="gradNW"     x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2dd4bf" stopOpacity={0.3}  /><stop offset="95%" stopColor="#2dd4bf" stopOpacity={0.02} /></linearGradient>
                </defs>
                ...
                <Area type="monotone" dataKey="totalAssets"      name="Assets"      stroke="#6366f1" strokeWidth={1.5} fill="url(#gradAssets)" dot={false} activeDot={{ r: 4, fill: "#6366f1", stroke: "#0f172a", strokeWidth: 2 }} />
                <Area type="monotone" dataKey="totalLiabilities" name="Liabilities" stroke="#f87171" strokeWidth={1.5} fill="url(#gradLiab)"   dot={false} activeDot={{ r: 4, fill: "#f87171", stroke: "#0f172a", strokeWidth: 2 }} />
                <Area type="monotone" dataKey="netWorth"         name="Net Worth"   stroke="#2dd4bf" strokeWidth={2.5} fill="url(#gradNW)"     dot={false} activeDot={{ r: 5, fill: "#2dd4bf", stroke: "#0f172a", strokeWidth: 2 }} />
...
const styles = {
  ...
  pageTitle: { fontFamily: "'DM Serif Display', serif", fontSize: 52, fontWeight: 400, color: "#f0f9ff", lineHeight: 1, letterSpacing: "-1px" },
  netWorthDisplay: { display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, padding: "16px 22px", background: "rgba(45,212,191,0.04)", border: "1px solid rgba(45,212,191,0.12)", borderRadius: 14 },
  netWorthLabel: { fontSize: 10, letterSpacing: "0.2em", color: "#0d9488" },
  netWorthValue: { fontFamily: "'DM Serif Display', serif", fontSize: 36, fontWeight: 400, letterSpacing: "-1px", lineHeight: 1 },
  ...
  rangeBtnActive: { background: "rgba(45,212,191,0.1)", borderColor: "rgba(45,212,191,0.3)", color: "#2dd4bf" },
  chartEmpty: { height: 240, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12 },
  chartEmptyIcon: { fontSize: 36, color: "#0f172a" },
  chartEmptyText: { fontSize: 13, color: "#1e293b", maxWidth: 320, textAlign: "center", lineHeight: 1.6 },
