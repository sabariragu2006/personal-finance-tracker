...
function DonutChart({ assetVal, liabilityVal, animate }) {
  ...
  if (total === 0) {
    return (<svg width="108" height="108" viewBox="0 0 108 108"><circle cx="54" cy="54" r="38" fill="none" stroke="#1e2433" strokeWidth="13" /></svg>);
  }
  const r = 38, circumference = 2 * Math.PI * r, gap = 4;
  ...
  return (
    <svg width="108" height="108" viewBox="0 0 108 108">
      ...
      <circle cx="54" cy="54" r={r} fill="none" stroke="#34d399" strokeWidth="13"
        strokeDasharray={`${assetLen} ${circumference}`} strokeDashoffset={0} strokeLinecap="round"
        style={{ transform: "rotate(-90deg)", transformOrigin: "54px 54px", transition: animate ? "stroke-dasharray 1.1s cubic-bezier(0.4,0,0.2,1)" : "none" }} />
      <circle cx="54" cy="54" r={r} fill="none" stroke="#f87171" strokeWidth="13"
        strokeDasharray={`${liabLen} ${circumference}`} strokeDashoffset={liabOffset} strokeLinecap="round"
        style={{ transform: "rotate(-90deg)", transformOrigin: "54px 54px", transition: animate ? "stroke-dasharray 1.1s cubic-bezier(0.4,0,0.2,1), stroke-dashoffset 1.1s cubic-bezier(0.4,0,0.2,1)" : "none" }} />
    </svg>
  );
}
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        .sidebar { width: 260px; max-height: 100vh; height: 100vh; background: #0d1117; display: flex; flex-direction: column; font-family: 'DM Sans', sans-serif; border-right: 1px solid #1a2030; overflow: hidden; position: fixed; top: 0; left: 0; z-index: 100; }
        .sidebar::before { content: ''; position: absolute; top: -60px; right: -60px; width: 180px; height: 180px; border-radius: 50%; background: radial-gradient(circle, rgba(52,211,153,0.07) 0%, transparent 70%); pointer-events: none; }
        .top-section { padding: 22px 16px 18px; border-bottom: 1px solid #1a2030; animation: fadeSlideIn 0.4s ease both; }
        ...
        .user-name { font-family: 'DM Serif Display', serif; font-size: 19px; color: #f0f4ff; letter-spacing: 0.01em; margin-bottom: 18px; line-height: 1.2; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .donut-row { display: flex; align-items: center; gap: 14px; margin-bottom: 14px; }
        .donut-wrap { position: relative; flex-shrink: 0; }
        .donut-center { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; pointer-events: none; }
        .donut-pct { font-family: 'DM Mono', monospace; font-size: 13px; font-weight: 500; line-height: 1; transition: color 0.4s; }
        .donut-label { font-size: 8px; color: #4a5568; text-transform: uppercase; letter-spacing: 0.08em; margin-top: 2px; }
        .legend { flex: 1; }
        ...
        .networth-strip { background: #0f1520; border: 1px solid #1a2030; border-radius: 10px; padding: 10px 13px; display: flex; align-items: center; justify-content: space-between; gap: 8px; }
        ...
        .nav { flex: 1; padding: 12px 10px; min-height: 0; display: flex; flex-direction: column; gap: 2px; animation: fadeSlideIn 0.5s ease both; animation-delay: 0.05s; overflow: hidden; }
        .nav-item { display: flex; align-items: center; gap: 11px; padding: 10px 11px; border-radius: 10px; cursor: pointer; transition: all 0.16s ease; color: #3a4a60; font-size: 13px; font-weight: 400; border: 1px solid transparent; position: relative; user-select: none; }
        ...
        .bottom-section { padding: 10px; border-top: 1px solid #1a2030; animation: fadeSlideIn 0.5s ease both; animation-delay: 0.1s; }
        .profile-card { display: flex; align-items: center; gap: 10px; padding: 10px 11px; border-radius: 10px; background: rgba(255,255,255,0.015); border: 1px solid #1a2030; transition: background 0.16s, border-color 0.16s; }
        ...
        .profile-card-active .avatar { box-shadow: 0 0 12px rgba(52,211,153,0.3); }
        .avatar { width: 34px; height: 34px; border-radius: 9px; background: linear-gradient(135deg, #34d399 0%, #0d9488 100%); display: flex; align-items: center; justify-content: center; font-family: 'DM Serif Display', serif; font-size: 12px; color: #0d1117; font-weight: 700; flex-shrink: 0; letter-spacing: 0.5px; }
        .profile-info { flex: 1; min-width: 0; }
        .profile-name { font-size: 12.5px; font-weight: 500; color: #c8d6e8; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .profile-email { font-size: 9px; color: #2a3a50; margin-top: 1px; font-family: 'DM Mono', monospace; letter-spacing: 0.04em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .error-banner { margin: 8px; padding: 8px 12px; background: rgba(248,113,113,0.08); border: 1px solid rgba(248,113,113,0.2); border-radius: 8px; font-size: 11px; color: #f87171; font-family: 'DM Mono', monospace; display: flex; align-items: center; justify-content: space-between; gap: 8px; cursor: pointer; }
        .auth-section { padding: 10px; border-top: 1px solid #1a2030; display: flex; flex-direction: column; gap: 6px; animation: fadeSlideIn 0.5s ease both; animation-delay: 0.1s; }
        .auth-btn { width: 100%; padding: 9px 14px; border-radius: 10px; font-family: 'DM Sans', sans-serif; font-size: 12.5px; font-weight: 500; cursor: pointer; transition: all 0.16s ease; display: flex; align-items: center; justify-content: center; gap: 7px; letter-spacing: 0.01em; }
        ...
        .auth-tagline { text-align: center; font-size: 9px; font-family: 'DM Mono', monospace; color: #1e2d3a; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 8px; padding-top: 2px; }
      `}</style>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 4 }}>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
                  <Skeleton w="70%" h={12} r={4} /><Skeleton w="90%" h={18} r={4} />
                  <Skeleton w="70%" h={12} r={4} /><Skeleton w="90%" h={18} r={4} />
                </div>
              </div>
              <Skeleton h={46} r={10} />
